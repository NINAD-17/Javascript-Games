...
class Player {
    ...
    strike(player, enemy, attackDmg) {
        ...
        return (`${player.name} attaks ${enemy.name} for ${damageAmount}`);
    }
    ...
    heal(player) {
        ...
        //  Update the game and DOM with updateGame()
        if(player === p1) {
            updateGame(player, p2, gameState);
        } else {
            updateGame(p1, player, gameState);
        }
...
let player1 = new Player("Json", 100, 6);
let player2 = new Player("Bson", 100, 7);
...
document.addEventListener('keydown', function (e) {
    ...
    const attackAud = document.getElementById("p2attack");
    attackAud.play();
...
document.addEventListener('keydown', function (e) {
    ...
    if(e.key == 'a' && !isOver && p1.health > 0) {
        p1.heal(p1);
    }

    // After healing then play heal sound
    const healSound = document.getElementById("p1heal");
    healSound.play();
